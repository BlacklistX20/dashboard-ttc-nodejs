...
async function saveCsps() {
  ...
  if (data instanceof Error) {
    ...
    const datetime = getDate();
    const sql = `INSERT INTO csps (updated_at, t1, h1, t2, h2, t3, h3, t4, h4, t_avg, h_avg) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
...
async function saveMsc() {
  ...
  if (data instanceof Error) {
    ...
    const datetime = getDate();
    const sql = `INSERT INTO msc (updated_at, t1, h1, t2, h2, t3, h3, t4, h4, t5, h5, t6, h6, t_avg, h_avg) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
...
async function updateLt2() {
  await updateTempData("http://192.168.10.72/data", 1);
  await updateTempData("http://192.168.10.78/data", 2);
  await updateTempData("http://192.168.10.73/data", 3);
  await updateTempData("http://192.168.10.76/data", 4);
  await updateTempData("http://192.168.10.77/data", 21); // Genset
  await updateTempData("http://192.168.10.77/data", 22); // Trafo
}
